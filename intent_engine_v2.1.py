"""
intent_engine.py  —  CNN-LSTM Intent Prediction Engine  (v2)
=============================================================
...
-----------
  ...
  BUG 8  — No camera fallback handled in the engine itself.
           ...
           first, then falls back to V4L2 index 10, then falls back to
           any available webcam, logging each attempt. The chosen camera
           type is exposed on engine.camera_backend.

Dependencies
------------
    # --- activate your venv before running any of these -------------------
    source ~/intent_env/bin/activate

    # --- system packages (apt — JetPack-optimised, CUDA/ARM-aware) --------
    sudo apt update
    sudo apt install -y python3-opencv python3-numpy v4l-utils

    # --- torch + torchvision (matched CUDA-enabled pair for JetPack 6) ----
    pip3 install http://jetson.webredirect.org/jp6/cu126/+f/5cf/9ed17e35cb752/torch-2.5.0-cp310-cp310-linux_aarch64.whl
    pip3 install http://jetson.webredirect.org/jp6/cu126/+f/5f9/67f920de3953f/torchvision-0.20.0-cp310-cp310-linux_aarch64.whl

    # pillow is pulled in automatically as a torchvision dependency

    # --- camera setup ------------------------------------------------------
    v4l2-ctl --list-devices          # find your camera index
    sudo chmod 777 /dev/video4       # replace 4 with your actual index

    # --- sanity check — all four must import and CUDA must print True ------
    python3 -c "import torch, torchvision, cv2, numpy; print('CUDA:', torch.cuda.is_available())"

    NOTES
    -----
    · Do NOT run `pip install torch` or `pip install torchvision`.
      The PyPI wheels are x86-only and will fail silently on Jetson ARM64.

    · Do NOT run `sudo apt install python3-torch` — the Ubuntu repo build
      is PyTorch 1.8 (2021) with no CUDA support. The pip3 wheel URLs above
      are the correct NVIDIA-built CUDA-enabled builds for JetPack 6 / Python 3.10.

    · pyrealsense2 is optional — the engine falls back to V4L2 automatically.
      If you need it, Intel's apt repo is required (pip install does not have
      an ARM64 build):
          sudo apt-key adv --keyserver keys.gnupg.net --recv-key F6E65AC044F831AC
          sudo add-apt-repository \
              "deb https://librealsense.intel.com/Debian/apt-repo $(lsb_release -cs) main"
          sudo apt update && sudo apt install -y python3-pyrealsense2

    · numpy 1.x ships with JetPack 6.2.2. Do not `pip install numpy` on top
      of it — mismatched versions cause silent dtype errors with torch tensors.
...
class _V4L2Camera:
    ...
    def __init__(self, index: int = 10,
...
class CameraFactory:
    ...
    2. V4L2 index 10           (confirmed Jetson setup)
    ...
    def open(width: int = 640, height: int = 480, fps: int = 30):
        ...
        try:
            ...
            log.warning("RealSense SDK unavailable (%s), trying V4L2 index 10 …", e)

        # 2. V4L2 index 10
        try:
            return _V4L2Camera(10, width, height, fps)
        except Exception as e:
            attempts.append(f"V4L2@10: {e}")
            log.warning("V4L2 index 10 failed (%s), trying V4L2 index 0 …", e)
